...
| object type              | entity accessor；有 identity 时写入 normalized field signal         |
...
| schema directive         | 只影响 codegen metadata；例如 deprecated、identity key、auth hint   |
...
- federation `@key` 或自定义 identity directive → entity identity metadata
...
`refs` 的元素是 entity reference，至少包含 `__typename` 和 identity key。这样 `User:1` 与 `Team:1` 不会冲突。
...
无 identity 的嵌套对象作为所属字段的值：

```txt
User:1.settings → Settings JSON value
```

不得为了追求深层响应式而递归拆分无 identity 对象。若应用需要把某个类型当实体，必须通过 codegen 配置或 schema metadata 明确 identity key。
