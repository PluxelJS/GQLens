...
当前 `cache.ts` 的 normalize 会优先使用 schema metadata；缺少 metadata 时才退回到 `__typename + id` 的通用识别。Entity / Value Object 规则见 [服务端 Schema 设计指南](../服务端-Schema设计指南.md)。
...
- 调 `plan()`。
- 调 fetcher。
- `writeOperationResult(store, result, operation.selections, ttl, metadata)`。
...
`applyInvalidations(store, invalidations, metadata?)` 支持：
...
`createMutationRunner({ store, mutation, fetcher, invalidate? })` 是 React/Solid 共享的 mutation 流程。
